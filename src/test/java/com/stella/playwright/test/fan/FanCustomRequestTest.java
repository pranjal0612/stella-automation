...
@Slf4j
public class FanCustomRequestTest extends BaseTest{
    String sheetName;
    ...
    public void createCustomRequestFanTest(String title, String amount,String cardNumber, String expiryDate, String cvc, String cardHolder,String country) {
        try{
            String domain = getDomain();
            Page page = getPage();
            FanCustomRequestPage fanCustomRequestPage = new FanCustomRequestPage(page, domain);
            fanCustomRequestPage.navigate();
            fanCustomRequestPage.setValue(title, amount, cardNumber, expiryDate, cvc, cardHolder, country);
        } catch (PlaywrightException e) {
            log.error("Error in createCustomRequestFanTest: ", e.getMessage());
            assertFalse(true,e.getMessage());
        }catch (Exception e) {
            log.error("An unexpected error occurred in createCustomRequestFanTest: ", e.getMessage());
            assertFalse(true,e.getMessage());
        }
    }
}
