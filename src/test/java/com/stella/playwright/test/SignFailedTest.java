...
@Slf4j
public class SignFailedTest extends BaseTest {
    ObjectMapper mapper;
    String sheetName;
    @Parameters({"sheetName"})
    @BeforeClass
    public void setValue(String sheetName){
        mapper = new ObjectMapper();
        this.sheetName = sheetName;
    }

    @DataProvider(name = "signup-data")
    public Object[][]realExcel(){
    ...
    void signUpFailedTest(String filePath,String name,String url, String email,String password ,int price , ITestContext context){
        try{
            ...
            signUpPage.signUpFailed();
        }catch (PlaywrightException e) {
            log.error("Playwright Error: " + e.getMessage());
            assertFalse(true, e.getMessage());
        } catch (Exception e) {
            log.error("error while testing signupFailedTest :: ", e);
            assertFalse(true);
        }
    }
}
